...
void buttonDebounce() {
  ...
  if (!(PIND & (1 << 4))) {
    ...
    if (!(buttonOldState & (1 << 0)) && (buttoncounts[0] > debounceCount)) {
      buttonActivated |= (1 << 0);
      buttonOldState |= (1 << 0);
    }
  } else {
  ...
  if (!(PIND & (1 << 3))) {
    ...
    if (!(buttonOldState & (1 << 1)) && (buttoncounts[1] > debounceCount)) {
      buttonOldState |= (1 << 1);
      buttonActivated |= (1 << 1);
    }
  } else {
...
boolean onButtonHeld(byte buttonSelect) {
  if (buttoncounts[buttonSelect] > holdCount) {
    return true;
  } else {
    return false;
  }
